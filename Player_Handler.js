      ...
      queue.textChannel
        .send({
          embeds: [
            new MessageEmbed()
              .setColor(ee.color)
              .setImage( `https://cdn.discordapp.com/attachments/1026147523678707803/1026440715028418560/standard_1.gif`
          )
              .setThumbnail(song.thumbnail)
              .setDescription(`>>> ** [\`${song.name}\`](${song.url}) **`)
              .addFields([
                {
                  name: `${emoji.song_by} เปิดโดย`,
                  value: `>>> ${song.user}`,
                  inline: true,
                },
                {
      ...
      queue.textChannel.send({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setDescription(
              `_ ${emoji.ERROR} โดนบางคนสกัดดาวรุ่ง_`
            )
            .setFooter({ text: ee.footertext, iconURL: ee.footericon }),
        ],
      });
      ...
      queue.textChannel.send({
        embeds: [
          new MessageEmbed()
            .setColor(ee.color)
            .setImage(
            `https://cdn.discordapp.com/attachments/1026147523678707803/1026440715028418560/standard_1.gif`
          )
            .setDescription(
              ` การเล่นจบแล้ว \n`
            )
          ...
          case "playp":
            {
              if (!channel) {
                ...
                send(
                  interaction,
                  `** ${emoji.ERROR} คุณต้องเข้าห้องเสียงที่มี 𝗣𝗔𝗣𝗔 𝗠𝗨𝗦𝗜𝗖🔐 **`
                );
    ...
    if (message.channel.id === channel.id) {
      if (message.author.bot) {
        ...
        if (!voiceChannel) {
          ...
          return send(message, `คุณต้องเข้าห้องเสียงที่มี 𝗣𝗔𝗣𝗔 𝗠𝗨𝗦𝗜𝗖🔐`);
        } else {
